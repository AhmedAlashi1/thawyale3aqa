...
<body style="direction:rtl;font-family: 'Cairo', sans-serif;margin: 10px;height: 100%">
@foreach($order as $order)
<div style="border: 1px solid blue; margin-bottom: 50px; padding: 50px;">
        ...
        <div style="float:right;">
            <table style="font-size: 14px;line-height: 1.2;padding-top: 5px;">
                ...
                <tr>
                    ...
                    <td style="padding-right: 15px;">
                        <?php
                        $sub = substr($order->address->mobile, 0, 5) ?? "";
                        $number = substr($order->address->mobile, 5) ?? "";
                        if($sub == '00965'){
                            echo $number ?? "";
                        }else{
                            echo $order->address->mobile ?? "";
                        }
                        ?>
                    </td>   </tr>
                <tr>
                ...
                <tr>
                    <td>طريقة التوصيل</td>
                    <td style="padding-right: 15px;"> {{$order->deliveryTypeTitle?$order->deliveryTypeTitle->title_ar: 'شحن دولي'}}

                    </td>
                </tr>
                @if($order->deliveryTypeTitle)
                    @if($order->deliveryTypeTitle->id == '1')
                        <tr>
                            <td>وقت التوصيل</td>
                            <td style="padding-right: 15px;"> {{$order->delivery_date .'  '.$order->time_id}}
                ...
                @else
                    <tr>
                        <td>وقت التوصيل</td>
                        <td style="padding-right: 15px;"> {{'شحن دولي'}}
                ...
                <tr>
                    <td>طريقة الدفع</td>
                    <td style="padding-right: 15px;"> {{$order->payment->title_ar ?? ""}}
                        @if($order->payment_id == 4 ||$order->payment_id == 3)
                ...
                <tr>
                    ...
                    @if($order->address)
                        @if($order->address->cityData ?? "")
                            <td style="padding-right: 15px;"> {{($order->address)?'('.$order->address->title.')'.'- المحافظة  '.$order->address->cityData->title_ar.'- المنطقة  '.$order->address->regionData->title_ar.'- القطعة  '.$order->address->block.'- اسم الشارع  '.$order->address->street.'- الجاده  '.$order->address->avenue.'- عماره  '.$order->address->building.'- الدور  '.$order->address->floor.'- رقم الشقة  '.$order->address->flat.'- ملاحظة '.$order->address->notes:''}}</td>
                        @else
                            <td style="padding-right: 15px;"> {{($order->address)?'('.$order->address->title.')'.'- المحافظة  '.$order->address->governate.'- المنطقة  '.$order->address->city.'- القطعة  '.$order->address->block.'- اسم الشارع  '.$order->address->street.'- الجاده  '.$order->address->avenue.'- عماره  '.$order->address->building.'- الدور  '.$order->address->floor.'- رقم الشقة  '.$order->address->flat.'- ملاحظة '.$order->address->notes:''}}</td>
                        @endif
                    @else
                        <td style="padding-right: 15px;"> {{($order->address)?'('.$order->address->title.')'.'- المحافظة  '.$order->address->governate.'- المنطقة  '.$order->address->city.'- القطعة  '.$order->address->block.'- اسم الشارع  '.$order->address->street.'- الجاده  '.$order->address->avenue.'- عماره  '.$order->address->building.'- الدور  '.$order->address->floor.'- رقم الشقة  '.$order->address->flat.'- ملاحظة '.$order->address->notes:'' }}</td>
                    @endif
    ...
    <div>
        <h3 style="text-align: center">
            ...
            <span style="display: block">تطبيق {{env('APP_NAME')}}  </span>
        </h3>
    </div>
    ...
    <div style="padding-left: 15px;padding-right: 15px;">
        <table style="width:100%;border: 1px solid black;border-collapse: collapse;text-align: center;font-size:14px">
            ...
            <tr>
                <td>تكلفة الطلب</td>
                <td colspan="4" style="text-align: left;direction: ltr;padding-left: 15px; "> {{number_format($cost,3)}} </td>
            </tr>
            <tr>
                <td>تكلفة التوصيل</td>
            ...
            @if($order->promo)
                <tr>
                    ...
                    @if($order->promo->type == 1)
                        <td colspan="7" style="text-align: left;direction: ltr;padding-left: 15px; "> -{{$order->promo->discount}} </td>
                    @else
                        <td colspan="7" style="text-align: left;direction: ltr;padding-left: 15px; "> -{{$order->promo->percent}}% </td>
                    @endif
                </tr>
            @endif
            ...
            <tr style="border: 1px solid black;border-collapse: collapse;">
                ...
                <td style="border: 1px solid black;border-collapse: collapse;" colspan="4" style="text-align: left;direction: ltr;padding-left: 15px; "> {{$order->notes}} </td>
            </tr>
            </tbody>
        </table>
    </div>
