...
var dataTable = $('#table_view').DataTable({
    ...
    "columns": [
        ...
        },
        {
            "data": function (row, type, val, meta) {
                var acti = row.action;
                return action_js_lang[acti];
            }, "orderable": false,
        },
        {
            "data": function (row, type, val, meta) {
                var mod = row.model;
                return admin_js_lang[mod];
            }, "orderable": false,
        },
        {
            "data": function (row, type, val, meta) {
        ...
        {"data": function (row) {
            return '<img src="' + assets_folder + '/global/img/flags/' + row.country_code.toLowerCase() + '.png"/> ' + row.ip_address;
        }, "orderable": false
        },
        {'data': function (row) {
             if(row.ids){
                 var outIds = '';
                 $.each(row.ids, function(k, v){
                     outIds += '#' + v + ' '
                 })
                 return outIds;
             }   
...
function viewDetails(obj) {
    try {
        ...
        var data = $.parseJSON(users_info[obj]);
...
$("#log_admin_delete_btn").click(function () {
