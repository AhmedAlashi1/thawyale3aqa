    ...
    "columns": [
        ...
        },
        {
            "data": function (row, type, val, meta) {
                var html = '<div style="width: 100%;text-align: center;">';
                    html += row.api.replace('_', ' ');
                return html;
            }, "orderable": true,
        },
        {
            "data": function (row, type, val, meta) {
                return typeof status_codes[row.status_code] != 'undefined' ? '(' + row.status_code + ') ' + status_codes[row.status_code].split('_').join(' ').capitalize() : 'unkown';
            }, "orderable": true,
        },
        {
            "data": function (row, type, val, meta) {
                return '<span class="' + (row.is_mobile ? '' : 'label label-danger') + '">' + (row.is_mobile ? 'جوال' : 'كومبيوتر') + '</span>';
            },
            "orderable": false,
        },
        ...
        {"data": "created_at", "orderable": true, },
        {
            ...
            "data": function (row, type, val, meta) {
                ...
                var html = '<div style="width: 100%;text-align: center;">';
                html += '<a title="الريكويست" class="btn btn-sm yellow" onclick="viewDetails(\'' + row._id + '\')"><i class="fa fa-edit"></i> الريكويست</a>';
                html += '</div>';
                return html;
            }
        },
        {
            "orderable": false,
            "data": function (row, type, val, meta) {
                users[row._id] = row.headers;
                var html = '<div style="width: 100%;text-align: center;">';
                html += '<a title="الهيدر" class="btn btn-sm purple" onclick="viewDetailsHeader(\'' + row._id + '\')"><i class="fa fa-edit"></i> الهيدر</a>';
                html += '</div>';
                return html;
            }
        },
...
function viewDetails(obj) {
    try {
        dialog_is.title = 'تفاصيل الريكويست';
        dialog_is.message = '<table class="table table-bordered" style="width: 100%;">' +
                '<tr><th>اسم الحقل</th><td>القيمة المرسلة</td></tr>';
        dialog_is.message += '<tr><th> الرابط</th><td><textarea class="form-control">' + users_info[obj].url + '</textarea></td></tr>';
        $.each(users_info[obj].request, function (key, val) {
            dialog_is.message += '<tr><th>' + key + '</th><td><textarea class="form-control">' + val + '</textarea></td></tr>';
        });
 ...
 * @param obj
 */
function viewDetailsHeader(obj) {
    try {
        dialog_is.title = 'تفاصيل الهيدر';
        dialog_is.message = '<table class="table table-bordered" style="width: 100%;">' +
                '<tr><th>اسم الحقل</th><td>القيمة المرسلة</td></tr>';
        $.each(users[obj], function (key, val) {
            dialog_is.message += '<tr><th>' + key + '</th><td><textarea class="form-control">' + val + '</textarea></td></tr>';
        });
        dialog_is.message += '</table>';
        delete dialog_is.buttons.success;
