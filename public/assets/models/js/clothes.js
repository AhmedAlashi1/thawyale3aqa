...
var dataTable = $('#table_view').DataTable({
    ...
    "ajax": {
        url: urls.get_data_url + '?cat_id=' + cat_id, // json datasource
        data: function (d) {
    ...
    "columns": [
        ...
        {
            "data": function (row, type, val, meta) {
                return row.cat ? row.cat.title_ar : ''
            }, "orderable": false
        },
        ...
        },
        {
            ...
            "data": function (row, type, val, meta) {
                ...
                html += '<a title="+100" class="btn btn-sm green" onclick="updateQuant(' + row.id + ',100)"><i class="fa fa-plus"></i> 100</a>';
                html += '<a title="تصفير" class="btn btn-sm purple" onclick="updateQuant(' + row.id + ',0)"><i class="fa fa-refresh"></i> تصفير</a>';
                html += '</div>';
...
dataTable.on('row-reorder', function (e, diff, edit) {
    ...
    dataTable.ajax.url(urls.get_data_url + "?ids=" + obj + "&pos=" + pos+ '&cat_id=' + cat_id).load();
});
...
function updateQuant(id, type) {
    bootbox.confirm('هل انت متاكد من تغيير فى كمية المنتج ؟', function (result) {
        if (result) {
            var my_data = {};
            my_data.id = id;
            my_data.status = type;
            $.post(urls.updateQuant, my_data, function (data) {
                if (data[0]) {
                    showNotify('success', model_js_lang[data[1]]);
                } else {
                    if (js_lang[data[1]]) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data[0] == true) {
                ...
                $(form1).find('input[name=order_limit]').val(data[2].order_limit);
                ...
                $(form1).find('input[name=barcode]').val(data[2].barcode);
...
$("#app_users_activate_btn").click(function () {
    if ($('input[name="ids[]"]:checked').length) {
        bootbox.confirm('هل انت متاكد من التفعيل', function (result) {
            if (result) {
                var ids = [];
...
$("#app_users_deactivate_btn").click(function () {
    if ($('input[name="ids[]"]:checked').length) {
        bootbox.confirm('هل انت متاكد من الغاء التفعيل', function (result) {
            if (result) {
                var ids = [];
                $.each($('table#table_view tbody input[type="checkbox"]:checked'), function (x, y) {
                    ids.push($(y).val());
